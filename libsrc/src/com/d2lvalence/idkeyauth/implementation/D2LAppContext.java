...
public class D2LAppContext implements ID2LAppContext {
        ...
        public URI createWebUrlForAuthentication(String host, int port, boolean encryptOperations, URI resultUri) {
            try {
                String scheme = encryptOperations ? Constants.URI_SECURE_SCHEME : Constants.URI_UNSECURE_SCHEME;
                URI uri = new URI(scheme+"://"+ host+":"+port+Constants.AUTHENTICATION_SERVICE_URI_PATH+"?"+buildAuthenticationUriQueryString( resultUri ));
        ...
        public ID2LUserContext createUserContext(URI uri, String hostName, int port, boolean encryptOperations) {
            ...
            if( userId == null || userKey == null ) {
             ...
             D2LUserContextParameters parameters = compileOperationSecurityParameters(
                    userId, userKey, hostName, port, encryptOperations );
        ...
        public ID2LUserContext createUserContext(String userId, String userKey, String hostName, int port, boolean encryptOperations) {
            D2LUserContextParameters parameters =
				compileOperationSecurityParameters( userId, userKey, hostName, port, encryptOperations );
        ...
        public ID2LUserContext createAnonymousUserContext(String hostName, int port, boolean encryptOperations) {
            D2LUserContextParameters parameters =
				compileOperationSecurityParameters( null, null, hostName, port, encryptOperations );
			return new D2LUserContext( parameters );
        }
        ...
        /**
         * Stores the provided parameters in OperationSecurityParameters so that they can be saved and loaded via serialization
         * 
         ...
         * @return 
         */
        private D2LUserContextParameters compileOperationSecurityParameters(
                String userId, String userKey, String hostName, int port, boolean encryptOperations ) {

                D2LUserContextParameters parameters = new D2LUserContextParameters(_appId,_appKey,userId,userKey,hostName,port,encryptOperations);
                return parameters;
        }
