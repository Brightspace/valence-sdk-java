...
public class MainTests {
    ...
    @Test
    public void appContext_CreateWebUrlForAuth_ReturnsURI_WhereScheme_IsHttps() {
        ID2LAppContext appContext = createAuthenticationSecurityObjectUnderTest();

        URI uri=null;
        try {
        uri=appContext.createWebUrlForAuthentication(TEST_HOST_NAME, TEST_PORT, new URI( TEST_API_URL ) );
    ...
    public void AuthContext_CreateWebUrlForAuth_ReturnsURI_WhereHost_MatchesInput() {
        ...
        try {
            URI uri = appContext.createWebUrlForAuthentication(TEST_HOST_NAME, TEST_PORT, new URI( TEST_API_URL ) );
    ...
    public void AuthContext_CreateWebUrlForAuth_ReturnsURI_WherePortNumber_MatchesInput() {
            ...
            ID2LAppContext appContext = createAuthenticationSecurityObjectUnderTest();
            try {
                URI uri = appContext.createWebUrlForAuthentication(TEST_HOST_NAME, expectedPortNumber, new URI( TEST_API_URL ) );
    ...
    public void AuthContext_CreateWebUrlForAuth_ReturnsUri_AndQueryParam_x_target_MatchesEncodedResultUrl() {
        ...
        try {
            ...
            URI uri = appContext.createWebUrlForAuthentication(
                    TEST_HOST_NAME, TEST_PORT, new URI( unencodedUrl ) );
    ...
    public void AuthContext_CreateOpContextFromResult_ReturnTypeIs_ID2LOperationSecurity() {
            ...
            ID2LUserContext result = appContext.createUserContext(
                    resultURI, TEST_HOST_NAME, TEST_PORT, true );

            assert result instanceof ID2LUserContext;
    }

    @Test
    public void AuthContext_CreateOpContextFromResult_HostName_IsTheSame() {
        ID2LAppContext appContext = createAuthenticationSecurityObjectUnderTest();
        URI resultURI= createResultURIWithUserIdAndKey( TEST_USER_ID, TEST_USER_KEY );
        ...
        Assert.assertEquals( TEST_HOST_NAME, userContext.getHostName() );
    }

    @Test
    public void AuthContext_CreateOpContextFromResult_PortNumber_IsTheSame() {
            final int expectedPortNumber = 44452;
            ID2LAppContext appContext = createAuthenticationSecurityObjectUnderTest();
            URI resultURI= createResultURIWithUserIdAndKey( TEST_USER_ID, TEST_USER_KEY );

            ID2LUserContext userContext  = appContext.createUserContext(resultURI, TEST_HOST_NAME, expectedPortNumber, true );

            Assert.assertEquals( expectedPortNumber, userContext.getPort() );
    }
    ...
    public void AuthContext_CreateOpContextFromResult_If_x_a_IsAbsentFromResultUri_ReturnsNull() {
        ID2LAppContext appContext = createAuthenticationSecurityObjectUnderTest();
        try {
            ID2LUserContext userContext  = appContext.createUserContext(new URI( TEST_API_URL ), TEST_HOST_NAME, TEST_PORT, true );
            Assert.fail();
        } catch(Exception e) {
    ...
    public void OpContext_CreateAuthUri_IfEncryptOpsFlagIsNotSet_UsesHttpScheme() {
        ...
        URI resultURI= createResultURIWithUserIdAndKey( TEST_USER_ID, TEST_USER_KEY );
        ID2LUserContext userContext  = appContext.createUserContext(
                resultURI, TEST_HOST_NAME, TEST_PORT, false );
    ...
    public void OpContext_CreateAuthUri_AuthUriHost_IsTheSameAs_OpContextHost() {
        ...
        URI resultURI= createResultURIWithUserIdAndKey( TEST_USER_ID, TEST_USER_KEY );
        ID2LUserContext userContext  = appContext.createUserContext(
                resultURI, expectedHostName, TEST_PORT, false );
    ...
    public void OpContext_CreateAuthUri_AuthUriPort_IsTheSameAs_OpContextPort() {
        ...
        URI resultURI= createResultURIWithUserIdAndKey( TEST_USER_ID, TEST_USER_KEY );
        ID2LUserContext userContext  = appContext.createUserContext(
                resultURI, TEST_HOST_NAME, expectedPort, false );
    ...
    /*@Test
    public void OpContext_CreateAuthUri_QueryParam_x_t_MatchesAdjustedTimestampInSeconds() {
            final String httpMethod = "PUT";
            final long serverClockSkewMilliseconds = 225000;
            SetTimestampProviderReturnValue( TEST_TIMESTAMP_MILLISECONDS - serverClockSkewMilliseconds );
            ID2LUserContext userContext  = createOperationSecurityObjectUnderTest();
            userContext.ServerSkewMillis = serverClockSkewMilliseconds;

            URI authURI= userContext.createAuthenticatedUri( TEST_API_PATH, httpMethod );

            String expectedTimestampParameter =TEST_TIMESTAMP_SECONDS.ToString( CultureInfo.InvariantCulture );
            String actualTimestampParameter = getURIQueryParameter( authUri, "x_t" );
            Assert.assertEquals( expectedTimestampParameter, actualTimestampParameter );
    }*/
    ...
    public void AuthContext_CreateOpContextFromUserValues_OpContext_CreateAuthUriMethod_CreatesSameUri() {
            ...
            ID2LUserContext userContextFromUserValues = appContext.createUserContext(
                    TEST_USER_ID, TEST_USER_KEY, TEST_HOST_NAME, TEST_PORT, true );
