...
/**
 * Stores the state of the application (app ID and app key) and provides methods for
 * authentication and for creating instances of ID2LUserContext with the appropriate information.
 */
public interface ID2LAppContext {
    
    /**
    * Provides the URL on the D2L server for users to authenticate against with HTTPS
    * 
    * @param host The host name of the server to authenticate to
    * @param port The port to authenticate with
    * @param resultUri The URI which the server should redirect to after the user has authenticated
    * @return A URI to redirect the user to
    */
    URI createWebUrlForAuthentication( String host, int port, URI resultUri );
 
    /**
    * Provides the URL on the D2L server for users to authenticate against, with the option to not use HTTPS
    * 
    * @param host The host name of the server to authenticate to
    * @param port The port to authenticate with
    ...
    * @return A URI to redirect the user to
    */
    URI createWebUrlForAuthentication( String host, int port, boolean encryptOperations, URI resultUri );

    /**
     * Creates an instance of ID2LUserContext with user parameters specified in the URI
     * 
     * @param uri The URI containing the user id and user key returned by the server
     ...
     * @return An instance of ID2LUserContext with user parameters specified in the URI
     * @see ID2LUserContext
     */
    ID2LUserContext createUserContext(
            URI uri, String hostName, int port, boolean encryptOperations );
     ...
     */
    ID2LUserContext createUserContext(
            String userId, String userKey, String hostName, int port, boolean encryptOperations );
